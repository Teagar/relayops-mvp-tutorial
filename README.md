# Teagarden na prática: do design ao MVP profissional

Tutorial visual e reproduzível de **Teagarden + OverClick + OpenCode**, usando uma execução real do início ao fim. O objetivo foi transformar o design aprovado em [`design/reference.svg`](./design/reference.svg) em um MVP funcional e testado, mantendo rastreabilidade desde o planejamento até o handoff.

> Este README é o tutorial principal de uso do Teagarden. Uma cópia editorial está disponível em [`TUTORIAL.md`](./TUTORIAL.md).

## O que você aprenderá sobre o Teagarden

- registrar e selecionar um workspace pelo MCP;
- criar uma missão `agentic` com objetivo e contexto;
- montar uma wave atômica de panes heterogêneos;
- definir o pane piloto e delegar briefs imutáveis;
- executar comandos exatos em PTYs com `write_pane`;
- observar estado sem depender do que está visível na UI;
- salvar resultados schema-v1, artefatos e evidências;
- validar e submeter handoffs autoritativos;
- lidar com blockers fail-closed;
- fechar panes e arquivar uma missão atomicamente;
- combinar o ciclo do Teagarden com cards e telemetria do OverClick.

## Começar rapidamente

```bash
git clone https://github.com/Teagar/relayops-mvp-tutorial.git
cd relayops-mvp-tutorial
npm install
npm run check
npm run dev
```

Abra `http://127.0.0.1:5173`. Para repetir a prova funcional em outro terminal:
...
## Ferramentas do Teagarden demonstradas

| Fase | Ferramentas |
| --- | --- |
| Descoberta e escopo | `list_workspaces`, `register_workspace`, `select_workspace`, `observe` |
| Missão | `create_mission`, `get_mission`, `complete_mission`, `close_and_archive_mission` |
| Orquestração | `spawn_panes`, `set_pilot_pane`, `write_pane`, `inspect_pane` |
| Delegação | `create_task`, briefs com caller/worker/pane/workspace/mission |
| Resultado | `save_result_draft`, `submit_handoff`, `get_handoff` |
| Segurança | descriptor privado, bridge stdio, IDs imutáveis, blockers sem escrita parcial |
